<template>
  <v-card tile class="my-5 grey lighten-4" elevation="10" :loading="loading">
    ...
    <v-form ref="form" class="pa-10">
      ...
      <v-layout>
        <v-flex>
          <label class="ml-3">Table Type *</label>
          <v-text-field
      ...
      <v-layout>
        <v-row v-if="singleRestaurantFloors">
          <v-col v-for="(floor, index) in singleRestaurantFloors" :key="index">
            <v-card
              class="mx-auto mt-6"
              max-width="344"
...
export default {
  ...
  computed: {
    ...
    ...mapGetters('restaurant', ['singleRestaurantFloors'])
  },
  methods: {
    ...mapActions('table', ['requestCreateTable']),
    updateImageUrl (url) {
      this.form.tableImg = url
    },
    async submitForm () {
      if (this.$refs.form.validate()) {
        console.log('create table-->> valid input', this.form)
        this.loading = true
        ...
        const res = await this.requestCreateTable({
          $apollo: this.$apollo,
          payload: this.form,
          jwt: this.jwt
        })
