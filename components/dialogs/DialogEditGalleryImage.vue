...
export default {
  ...
  created () {
    this.$nuxt.$on(DIALOG_EVENT.TOGGLE_EDIT_GALLERY_DIALOG, (galleryImage) => {
      this.dialog = !this.dialog
      console.log(galleryImage)
      this.GalleryId = galleryImage.id
      this.form.image = galleryImage.image
      this.form.description = galleryImage.description
      this.form.type = galleryImage.type
    })
  },
  methods: {
    ...mapActions('gallery', ['requestEditGalleryImage']),
    updateImageUrl (url) {
      this.form.image = url
    },
    async submitForm () {
      if (this.$refs.form.validate()) {
        ...
        this.loading = true
        const res = await this.requestEditGalleryImage({
          $apollo: this.$apollo,
          payload: this.form,
